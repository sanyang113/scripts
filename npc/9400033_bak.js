...
var items = [
	5800000,5800001,5800002,5800006
];
...
function action(mode, type, selection) {
    ...
    switch(status) {
        case 0:
            ...
            var msg = "這是山羊谷高級修練場，請問您想選擇哪個怪物修練呢？\r\n#r※請注意：訓練場VIP券購買後即開始倒數計時，請謹慎購買。#k\r\n#r※請注意：修練場裡面不會掉30%卷軸跟怪物卡。#k";
            for (var i = 0; i < mobs.length; i++) {
				msg += "\r\n#L" + i + "# #b"  + cm.getIcon(mobs[i][1]) + " " + cm.getMobName(mobs[i][0]) + "的房間";
			}
			cm.sendOk(msg); 
            break;
        case 1:
        ...
        case 2:
            ...
            var mobid = mobs[selMob][0];

            if(selItem == 99) {
                if(cm.getPlayer().getMonsterBook().getLevelByCard(mobs[selMob][1]) < 1) {
                    cm.sendOk("很抱歉，您的卡片數不足，無法進入。");
                    cm.dispose();
                    return;
                }
                var msg = "您擁有" + cm.getIcon(mobs[selMob][1]) + "卡片，目前有 " + cm.getPlayer().getMonsterBook().getLevelByCard(mobs[selMob][1]) +" 張，請問您確定要進入嗎？";
                cm.sendYesNo(msg);
                break;
            }
        ...
        case 3:
            ...
            cm.removeMonsterCard(mobs[selMob][1]);
            cm.dispose();
            return;
        default:
            cm.dispose();
