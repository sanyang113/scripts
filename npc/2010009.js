...
function action(mode, type, selection) {
	...
	if (status == 0) {
	    ...
	    cm.sendYesNo("這個 #b"+ guildName + "#k 是你想要的公會聯盟名字嗎？？");
			...
			var other = cm.getPlayer().getClient().getChannelServer().getPlayerStorage().getCharacterById(pt.getMemberByIndex(1).getId());
			if(other.getId() == cm.getPlayer().getId()) {
				cm.sendOk("創建聯盟時的組隊不得使用隊長移轉，請您解散組隊並由您重新建立組隊");
				cm.dispose();
				return;
			}
