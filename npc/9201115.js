		...
		if (em.getProperty("glpq6").equals("0")) {
			...
			if (cm.getMap().getAllMonstersThreadsafe().size() == 0) {
				if (status == 0) {
					...
					cm.getMap().spawnMonsterOnGroundBelow(mob, new java.awt.Point(-22, 276));
					...
					var mob4 = em.getMonster(9400589);
					cm.getMap().spawnMonsterOnGroundBelow(mob4, new java.awt.Point(-496, 276));
			...
			if (cm.getMap().getAllMonstersThreadsafe().size() == 0) {
				...
				cm.broadcastFinishEvent("CWK-守護者城堡");
				cm.addCountSquad(9400591);
				cm.dispose();
