...
function action(mode, type, selection) {
	...
	if (cm.getPlayer().getMarriageId() <= 0) {
		...
		cm.gainItemPeriod(5090100,num,1);
		//chr.getClient().gainItemPeriod(5090100,(num / 2),1); 
		var partnerNum = Math.floor(num / 2);
		cm.gainItemPeriod(chr, 5090100, partnerNum, 1); // 給伴侶（改成傳入 chr）
	    } else {
