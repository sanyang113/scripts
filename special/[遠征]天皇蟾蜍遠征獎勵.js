...
function action(mode, type, selection) {
    ...
    switch(status) {
        case 0:
            var items = cm.searchMob(9400409);
            var msg = "#b天皇蟾蜍遠征#k 可取得獎勵如下:\r\n\r\n";
                ...
                for(var i = 0; i < items.length; i++) {
                    if(items[i][0] != 0) {
                        if(items[i][1]==0 && items[i][0]>=2450000 && items[i][0]<2460000 ){msg += "#b#i" + items[i][0] + ":#"}}}
                for(var i = 0; i < items.length; i++) {
                    if(items[i][0] != 0&& items[i][0] != 2340000) {
                        if (items[i][1]==0 && items[i][0]>=2000000 && items[i][0]<2040000 ||
                            ...
                            items[i][1]==0 && items[i][0]>=2300000 && items[i][0]<2450000 ||
                            items[i][1]==0 && items[i][0]>=2460000 && items[i][0]<3000000
                        ){msg += "#b#i" + items[i][0] + ":#"}}}
            cm.sendOk(msg);
            cm.dispose();
            return;
