...
function action(mode, type, selection) {
    ...
    switch(status) {
        case 0:
            ...
            for (var i = 0; i < 100; i++) {
                var itemid = -1;
                if (cm.getInventory(4).getItem(i) != null && cm.isDivination(itemid = cm.getInventory(4).getItem(i).getItemId())) {
                    msg2 += "\r\n#L" + itemid + "#交換#b#i" + itemid + ":##t" + itemid + "##k";
                }
            }
            if(msg2 == "") {
                cm.sendOk("很抱歉，您身上沒有命運卡");
                cm.dispose();
                return;
            }
        ...
        case 1:
            cm.dispose();
            cm.openNpcDivination(cm.getNpc(),selection);
            break;
        default:
            cm.dispose();
            return;
