...
function action(mode, type, selection) {
    ...
    switch(status) {
        case 0:
            ...
            break;
        case 1:
            var instance = cm.getMapleCandyGame();
            if(selection == 1) {
                cm.dispose();
                ...
                cm.getPlayer().changeMap(map, map.getPortal(1));
                return;
            } else if(selection == 6) {
                var map = cm.getPlayer().getClient().getChannelServer().getMapFactory().getMap(109020007);
                if(map == null) {
                    cm.sendOk("無法傳送到該地圖");
                    cm.dispose();
                    return;
                ...
                for (var i = 0; i < chrs.length; i++) {
                    var chr = chrs[i];
                    chr.changeMap(map, map.getPortal(chr.isGM() ? 1 : 0));
                }
                cm.dispose();
                return;
            } else if(selection == 7) {
