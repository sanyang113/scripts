...
function action(mode, type, selection) {
    ...
    switch(status) {
        case 0:
            var items = cm.searchMob(8800002);
            var msg = "#b殘暴炎魔遠征#k 可取得獎勵如下:\r\n\r\n";
                ...
                for(var i = 0; i < items.length; i++) {
                    if(items[i][0] != 0) {
                        if(items[i][0]>=2280000 && items[i][0]<2290000){msg += "#b#i" + items[i][0] + ":#"}}}
                for(var i = 0; i < items.length; i++) {
                    if(items[i][0] != 0) {
                        if(items[i][1]==0 && items[i][0]>=2290000 && items[i][0]<2300000){msg += "#b#i" + items[i][0] + ":#"}}}
                for(var i = 0; i < items.length; i++) {
                    if(items[i][0] != 0) {
